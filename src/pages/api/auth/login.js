...
export default async function handler(req,res){
    if (req.method == "POST"){
        ...
        try {
            ...
            const [userInfo] = await connectDB.query("select user_id, name, role, password from user where nickname=? ",[nickname])

            // user 정보가 없으면
            if(userInfo ==0){
                res.status(200).json({message : "failed", error : "no user"})
            ...
            if(userInfo.length >0 ){
                ...
                if(passwordMatch){
                    ...
                    const token = jwt.sign(user, secretKey, { expiresIn: '1h' }); // 토큰 만료 시간 설정 (예: 1시간)
                    // 헤더에 jwt값 쿠키로 넣어 전송
                ...
                //비밀번호 불일치
                    ...
                    res.status(401).json({ message: "failed", error: "비밀번호가 다릅니다." });
